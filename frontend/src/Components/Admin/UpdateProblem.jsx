...
const UpdateProblem = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      alert("Problem update failed. Check console for details.");
    }
  };

  if (loading) return <div className="text-center mt-10">Loading problem...</div>;

  return (
    <div className="container mx-auto px-4 py-8 max-w-6xl space-y-8">
      <h1 className="text-4xl font-extrabold text-center text-primary-content">Update Problem</h1>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <button type="submit" className="btn btn-primary btn-lg w-full">🚀 Update Problem</button>
      </form>
    </div>
  );
};
